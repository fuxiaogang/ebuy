...
// 			$('[class ^= "childNav_"]').hide();
// 			$('.childNav_' + $(this).attr('item-id')).show();
// 		});
	});
</script>
</head>
...
<div class="navbar">
	<ul class="clearfix">
		<li class="${empty s_product  ? 'current' : ''}"><a href="index.jsp">首页</a>
		</li>
		<c:forEach var="bigType" items="${bigTypeList }">
			<li class="${bigType.id eq bigTypeId  ? 'current' : ''}">
				<a href="product.action?s_product.bigType.id=${bigType.id }"  item-id='${bigType.id }' class="mainNav">${bigType.name }</a>
			</li>
		</c:forEach>
	</ul>
</div>
 
<div id="childNav" class="childNav_0" style="${empty s_product  ? 'display:block' : 'display:none'}"  >
	<div class="wrap">
		<ul class="clearfix">
...
<c:forEach var="bigType"  items="${bigTypeList }" varStatus="status"> 
	<div id="childNav" class="childNav_${bigType.id }" style="${bigType.id eq bigTypeId  ? 'display:block' : 'display:none'}"  >
		<div class="wrap">
			<ul class="clearfix">
